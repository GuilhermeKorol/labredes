...
int main(int argc, char *argv[]){
	...
	printf("\n Out of the loop\n");
	// Tentar enviar daqui alguma pacote ao server
	frame_len = 0;
	eh_out->ether_dhost[0] = mac_dst[0];
	eh_out->ether_dhost[1] = mac_dst[1];
	eh_out->ether_dhost[2] = mac_dst[2];
	eh_out->ether_dhost[3] = mac_dst[3];
	eh_out->ether_dhost[4] = mac_dst[4];
	eh_out->ether_dhost[5] = 0x02;
	socket_address.sll_addr[0] = mac_dst[0];
	socket_address.sll_addr[1] = mac_dst[1];
	socket_address.sll_addr[2] = mac_dst[2];
	socket_address.sll_addr[3] = mac_dst[3];
